...
class Table:
    ...
    def __init__(self, name, num_columns, key):
        ...
        self.thrd = RepeatedTimer(.1, self.__merge)
            ...
            if page_range.has_updated != recent_tail:
                ...
                for i in range(len(merge_base_page_list)):
                    ...
                    for j, rid in enumerate(merge_base_page[RID_COLUMN], 0):
                        ...
                        if (rid_tail != None and page_range.has_updated != None and int(rid_tail[1:]) <= int(
                                page_range.recent_tail[1:])):
                            print('z')
                            if (rid != -1):
                                for k in range(self.table.num_columns):
                                    ...
                                    if schema_encoding[k] == 1:
                                        merge_base_page[4 + k][j].write(
                                            page_range.tail_page_list[location_tail[0]].tail_page_list[
                                                location_tail[1]][4 + k].read(location_tail[2]))

                    merge_base_page[INDIRECTION_COLUMN] = page_range.base_page_list[i][INDIRECTION_COLUMN].copy()
                    merge_base_page[SCHEMA_ENCODING_COLUMN] = page_range.base_page_list[i][
                        SCHEMA_ENCODING_COLUMN].copy()
